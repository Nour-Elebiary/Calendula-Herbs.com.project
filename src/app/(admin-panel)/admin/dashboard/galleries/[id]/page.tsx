...
export default function GalleryItemsPage() {
  ...
  const addMedia = async (mediaFileId: string) => {
    ...
    const res = await fetch(`/api/admin/gallery/${id}/items`, {
      ...
      body: JSON.stringify({ type, mediaFileId, title }),
    })
    if (res.ok) { toast.success('Item added'); setAddOpen(false); setTitle(''); fetchGallery() }
  ...
  const addExternal = async () => {
    ...
    const res = await fetch(`/api/admin/gallery/${id}/items`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ type, externalUrl, title }),
    })
    if (res.ok) { toast.success('Item added'); setAddOpen(false); setExternalUrl(''); setTitle(''); fetchGallery() }
    else toast.error('Failed to add item')
    setSaving(false)
  }
